...
import '../../../ar_navigation/presentation/widgets/ar_camera_preview.dart';
...
class _NavigationMapViewState extends State<_NavigationMapView>
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final arEnabled =
        context.watch<NavigationBloc>().state is NavigationReady &&
        (context.watch<NavigationBloc>().state as NavigationReady)
            .isArViewEnabled;
    ...
    return Column(
      children: [
        StepIndicator(
          currentStep: 3,
          title: arEnabled ? 'AR Navigation' : 'Direct Guidance',
          onBack: () =>
        ...
        Expanded(
          child: Stack(
            children: [
              // ── AR Background (Only when enabled) ───────────────────────
              if (arEnabled) Positioned.fill(child: ArCameraPreview()),

              // ── Map (Normal or Overlay) ──────────────────────────────────
              Opacity(
                opacity: arEnabled ? 0.4 : 1.0,
                child: MapView(
              ...
              Positioned(
                left: 16,
                bottom: 140, // Positioned above offset button
                child: FloatingActionButton.small(
                  onPressed: () {
                    context
                        .read<NavigationBloc>()
                        .add(ToggleArViewEvent(!arEnabled));
                  },
                  backgroundColor:
                      arEnabled ? theme.colorScheme.primary : theme.colorScheme.surface,
                  foregroundColor:
                      arEnabled ? theme.colorScheme.onPrimary : theme.colorScheme.primary,
                  heroTag: 'ar_toggle_fab',
                  child: Icon(arEnabled ? Icons.view_in_ar : Icons.map),
                ),
              ),
